import { call, put, getContext } from 'redux-saga/effects'
import utils from 'shared/utils'
import { deleteApiFailure, deleteApiSuccess } from 'store/actions/api'

export default function* deleteApiWorker(action) {
  try {
    const { rest } = yield getContext('api')
    const payload = yield call(utils.removeEmptyValues, action.payload)
    const { data, status } = yield call(rest.backend.deleteApi, payload)
    const result = {
      ...data,
      statusCode: status
    }
    data.statusCode = status
    yield put(deleteApiSuccess(result))
  } catch (error) {
    yield put(deleteApiFailure(error))
  }
}
